...
class Astar_DDC(Graph):
    ...
    def search(self):
        ...
        if self.visualize:
            ...
            self.ax.add_patch(patches.Circle(
                (self.start[0], self.start[1]), radius=0.1, linewidth=1, alpha=0.5, edgecolor='r', facecolor='r'))
            self.ax.add_patch(patches.Circle(
                (self.goal[0], self.goal[1]), radius=0.1, linewidth=1, alpha=0.5, edgecolor='g', facecolor='g'))
        ...
        costMatrix = np.array([[[np.inf for k in range(threshold_angle)] for j in range(
            int(self.h/threshold))] for _ in range(int(self.w/threshold))])
        ...
        for action in self.actions:
            ...
            if (self.isInsideArena(x, y)) and (not self.insideObstacle(x, y)):
                ...
                new_node = Node(new_state, node, action,
                                node.cost + step_cost, path_array)
    ...
    def reachedTarget(self, current_state, thresh_radius):
        ...
        """
        radius_sq = np.square(
            current_state[0] - self.goal[0]) + np.square(current_state[1] - self.goal[1])
...
if __name__ == "__main__":
    ...
    Parser.add_argument('--init_state', default="1, 1, 0",
                        help='initial state of the puzzle')
    Parser.add_argument('--goal_state', default="9, 9, 0",
                        help='goal state to be reached')
